...
from api.v1.rating.models import UserFilmRating
...
class RatingAPIView:
    ...
    async def rate_film(
        self, request: Request, film_id: UUID = Body(), stars: int = Body(ge=0, le=10)
    ) -> None:
    ...
    async def film_rating(self, film_id: UUID = Body()) -> UserFilmRating:
