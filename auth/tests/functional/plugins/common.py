...
def make_post_request():
    async def inner(service_url: AnyUrl, endpoint: str, query_data: dict | None = None, token: str = None):
        session = aiohttp.ClientSession()
        url = service_url + endpoint
        if token:
            headers = {f"Authorization": f"Bearer {token}"}
        ...
        async with session.post(url, headers=headers, data=json.dumps(query_data)) as response:
            ...
            response = {"headers": response.headers, "status": response.status, "body": body}
        await session.close()
        logging.info(f"Got response with code {response['status']}")
        return response
